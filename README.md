...
> the user habits dashboard or table molds itself over time. For that we deal with the `handling of Data` in both frontend and backend.
> In addition we use the `Push API` which gives web applications the ability to receive notifications sent to them from a server, whether
